...
% https://github.com/TheresaHartmann/CherISH_ProjectDC6_Application/tree/main
...
figure(), plot(t, noise), ylabel("Amplitude"), xlabel("Time [s]"), title("Gaussian white noise burst"); % plot signal
...
azimuth1 = [180 0 -180];	% azimuth angles in degrees -> start/end behind the head
...
% Plot the trajectories and the signal
plot_trajectory(spat_signal1, hrtf.Data.SamplingRate, spat_azimuth1, spat_elevation1, spat_radius1, '2.1.	Horizontal clockwise rotation')
...
% Plot the trajectories and the signal
plot_trajectory(spat_signal2, hrtf.Data.SamplingRate, spat_azimuth2, spat_elevation2, spat_radius2, '2.2.	Approach from left');
...
% Plot the trajectories and the signal
plot_trajectory(spat_signal3, hrtf.Data.SamplingRate, spat_azimuth3, spat_elevation3, spat_radius3, '2.3.	Approach from front')

% The radius decrease from 1 m to 0.2 m like the plot of the trajectories
% shows. The signal should be getting louder, so amplitude shoud increase
% but it decreases (cf. figure 7).
